...
class Player(BasePlayer):
    ...
    Q3 = models.IntegerField(
        label="3. 如果您認為本回合抽出 100 筆資料中有 70 筆來自紅罐，並且填答如下圖。請問以下敘述何者「錯誤」？ ",
        #label="",
        choices=[
            [0, 'A. 如果電腦隨機抽到第 6 行（紅框處）作為決定報酬的依據，本回合會採用紅罐抽獎。則根據題幹中您認為的情況，得到 200 法幣的機率為 70/100 。'], 
            [1, 'B. 如果電腦隨機抽到第 7 行（黃框處）作為決定報酬的依據，本回合會採用隨機抽獎，您得到 200 法幣的機率為 65/100 。'], 
            [2, 'C. 如果將第 7 行（黃框處）的填答改成「採用紅罐抽獎」，根據題幹中您認為的情況，若電腦隨機抽中該行作為決定報酬的依據，您得到 200 法幣的機率為 70/100 。'], 
